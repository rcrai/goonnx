...
OrtCreateSessionOptionsResponse createSessionOptions(
  ...
  if (params->optimizedModelFilePath != NULL) {
    status = api->SetOptimizedModelFilePath(sessionOptions,
                                            params->optimizedModelFilePath);
    if (status != NULL) {
  ...
  if (params->executionMode != 0) {
    status =
        api->SetSessionExecutionMode(sessionOptions, params->executionMode);
    if (status != NULL) {
      return releaseAndRespondErrorStatus(api, sessionOptions, status);
    }
  }
  ...
  if (params->logVerbosityLevel > 0) {
    status = api->SetSessionLogVerbosityLevel(sessionOptions,
                                              params->logVerbosityLevel);
    if (status != NULL) {
      return releaseAndRespondErrorStatus(api, sessionOptions, status);
    }
  }
  if (params->logSeverityLevel > 0) {
    status = api->SetSessionLogSeverityLevel(sessionOptions,
                                             params->logSeverityLevel);
    if (status != NULL) {
      return releaseAndRespondErrorStatus(api, sessionOptions, status);
    }
  }
  if (params->graphOptimizationLevel != DefaultGraphOptimizationLevel) {
    status = api->SetSessionGraphOptimizationLevel(
        sessionOptions, params->graphOptimizationLevel);
    if (status != NULL) {
      return releaseAndRespondErrorStatus(api, sessionOptions, status);
    }
  }
  if (params->intraOpNumThreads != DefaultIntraOpNumThreads) {
    status =
        api->SetIntraOpNumThreads(sessionOptions, params->intraOpNumThreads);
    if (status != NULL) {
      return releaseAndRespondErrorStatus(api, sessionOptions, status);
    }
  }
  if (params->interOpNumThreads != DefaultInterOpNumThreads) {
    status =
        api->SetInterOpNumThreads(sessionOptions, params->interOpNumThreads);
    if (status != NULL) {
      return releaseAndRespondErrorStatus(api, sessionOptions, status);
    }
  }
  if (params->numCustomOpDomains > 0 && params->customOpDomains != NULL) {
    for (int i = 0; i < params->numCustomOpDomains; i++) {
      status =
          api->AddCustomOpDomain(sessionOptions, params->customOpDomains[i]);
      if (status != NULL) {
        return releaseAndRespondErrorStatus(api, sessionOptions, status);
      }
    }
